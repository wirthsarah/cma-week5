...
ggplot()+
  ...
  geom_point(filter(beide,meet==TRUE),mapping=aes(x=E.Sabi,y=N.Sabi),pch=21, alpha=0.7,fill="#00BFC4", color="black")+
  geom_point(filter(beide,meet==TRUE),mapping=aes(x=E.Rosa,y=N.Rosa),pch=21, alpha=0.7,fill="#F8766D", color="black")
